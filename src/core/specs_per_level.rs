...
impl Plugin for SpecsPerLevelPlugin {
    fn build(&self, app: &mut App) {
        app.register_type::<SpecsPerLevel>();
    }
}
...
impl Default for SpecsPerLevel {
    fn default() -> Self {
...
impl SpecsPerLevel {
    fn from_levels_and_specs(
        levels_and_specs: impl IntoIterator<Item = (usize, Spec)>,
        default: Spec,
    ) -> Self {
        SpecsPerLevel {
...
/// The difficulty specifications for a level
#[derive(Reflect, Copy, Clone, Debug, Default, PartialEq)]
pub struct Spec {
    /// What fruit to spawn
    pub fruit_to_spawn: Fruit,
    /// Modifier for pacmans speed if no energizer is active
    pub pacman_normal_speed_modifier: f32,
    /// Modifier for pacmans speed if an energizer is active
    pub pacman_frightened_speed_modifier: f32,
    /// Modifier for the ghost speed if no energizer is active
    pub ghost_normal_speed_modifier: f32,
    /// /// Modifier for the ghost speed when going through a tunnel
    pub ghost_tunnel_speed_modifier: f32,
    /// Modifier for the ghost speed it is in the frightened state
    pub ghost_frightened_speed_modifier: f32,
    /// If the remaining dots are greater or equal to this value, Blinky goes into elroy phase 1
    pub elroy_1_dots_left: usize,
    /// Modifies the speed of Blinky when in elroy phase 1
    pub elroy_1_speed_modifier: f32,
    /// If the remaining dots are greater or equal to this value, Blinky goes into elroy phase 2
    pub elroy_2_dots_left: usize,
    /// Modifies the speed of Blinky when in elroy phase 2
    pub elroy_2_speed_modifier: f32,
    /// How long will an energizer be active
...
mod tests {
    ...
    fn it_returns_the_right_spec_for_a_given_level() {
        ...
        let specs_per_level = SpecsPerLevel::from_levels_and_specs([(1, spec)], Spec::default());
