    ...
    fn build(&self, app: &mut App) {
        app.insert_resource(Level(1))
            .add_systems(
                ...
                (HighScoreSerializable::save, increase_level),
            )
            .add_systems(OnExit(Game(GameOver)), reset_level)
            .add_systems(OnEnter(MainMenu(Menu)), reset_level);
    }
}

fn increase_level(mut level: ResMut<Level>) {
    level.increase();
}

fn reset_level(mut level: ResMut<Level>, config: Res<Config>) {
    // Set from config
